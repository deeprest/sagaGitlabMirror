...
public class BooleanMesh
{
  ...
  public BooleanMesh( Collider2D A, Collider2D B )
  {
  ...
  class intersectionDATA
  {
    ...
    public Collider2D meshColliderB;
    public int triangleA;
    public float customDistance;
    public Ray r1, r2;
    public RaycastHit2D hit;

    public intersectionDATA( Triangulation a, Triangulation b, Collider2D m )
    {
  ...
  void intersectionRay( int originVertice, int toVertice, intersectionDATA var )
  {
    ...
    if( var.A.vertices[var.A.triangles[var.triangleA].indexVertice[originVertice]].type == 0 && var.meshColliderB.Raycast( var.r1.direction, hits, var.customDistance ) > 0 )
    {
      var.hit = hits[0];
      intersectionPoint( var );
    }
    if( var.A.vertices[var.A.triangles[var.triangleA].indexVertice[toVertice]].type == 0 && var.meshColliderB.Raycast( var.r2.direction, hits, var.customDistance ) > 0 )
    {
      var.hit = hits[0];
      intersectionPoint( var );
    }
  ...
  bool ConcaveHull( Collider2D meshCollider, Vector3 position, Ray ray, RaycastHit hit )
  {
  ...
  bool In( Collider2D meshCollider, Vector3 position )
  {
